...
async function confirmPrinterWasAdded(previousMatches, values) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (countMatchingPrinters(items, values) > previousMatches) return true;
  }
  return false;
}
...
addPrinterForm.addEventListener('submit', async event => { event.preventDefault(); const saveButton = document.querySelector('#saveAddPrinter'); const values = Object.fromEntries(Object.entries(addFieldIds).map(([key,id]) => [key, document.querySelector(`#${id}`).value.trim()])); const previousMatches = countMatchingPrinters(items, values); saveButton.disabled = true; saveButton.textContent = 'กำลังเพิ่ม…'; addPrinterMessage.classList.remove('error'); addPrinterMessage.textContent = ''; try { await fetch(sheetWriteUrl, { method:'POST', mode:'no-cors', headers:{'Content-Type':'text/plain;charset=utf-8'}, body:JSON.stringify({ action:'add', values }) }); addPrinterMessage.textContent = 'ส่งข้อมูลเรียบร้อยแล้ว'; await new Promise(resolve => setTimeout(resolve, 180)); closeAddPrinterDialog(); void confirmPrinterWasAdded(previousMatches, values).then(confirmed => { if (!confirmed) { syncStatus.textContent = 'ส่งข้อมูลแล้ว • Google Sheets กำลังอัปเดตในเบื้องหลัง'; syncStatus.classList.remove('error'); } }); } catch (error) { addPrinterMessage.textContent = error.message || 'เพิ่มข้อมูลไม่สำเร็จ กรุณาตรวจการเชื่อมต่อ Google Apps Script'; addPrinterMessage.classList.add('error'); } finally { saveButton.disabled = false; saveButton.textContent = 'เพิ่มข้อมูล'; } });
render();
syncGoogleSheet();
