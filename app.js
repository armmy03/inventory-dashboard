...
[search, departmentFilter, floorFilter, modelFilter, assetFilter].forEach(control => control.addEventListener('input', () => { currentPage = 1; render(); }));
buildingFilter.addEventListener('input', () => { currentPage = 1; refreshFloorOptions(); render(); });
document.querySelector('#clearFilters').addEventListener('click', () => { currentPage = 1; search.value = ''; departmentFilter.value = ''; buildingFilter.value = ''; modelFilter.value = ''; assetFilter.value = ''; refreshFloorOptions(); render(); });
